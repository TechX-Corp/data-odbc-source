...
impl NativeType {
    pub fn to_native_type_num(&self) -> dpiNativeTypeNum {
        match *self {
            ...
            // NativeType::Number => DPI_NATIVE_TYPE_DOUBLE,
...
impl OracleType {
    pub(crate) fn from_type_info(info: &dpiDataTypeInfo) -> Result<OracleType, Error> {
    ...
    pub(crate) fn var_create_param(&self) -> Result<(u32, NativeType, u32, i32), Error> {
        ...
        match *self {
            ...
            OracleType::Number(p, s) => {
                if (p <= 38) & (s >= 0) & ((s as u8) <= p) {
                    Ok((DPI_ORACLE_TYPE_NUMBER, NativeType::Number, 0, 0))
                } else {
                    Ok((DPI_ORACLE_TYPE_NUMBER, NativeType::Number, 0, 0))
                }
            }
            OracleType::Float(_) => Ok((DPI_ORACLE_TYPE_NUMBER, NativeType::Number, 0, 0)),
